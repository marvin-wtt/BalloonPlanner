...
def solve(
    ...
    vehicles: List[Dict[str, Any]] = (
            [dict(**b, kind="balloon") for b in balloons]
    ...
    flights_so_far = {
        p_id: people_by_id[p_id]["flights"] for p_id in person_ids
    }
    nationality = {
    ...
    for v in vehicle_ids:
        if max_weight[v] > 0:
            model.Add(
                sum(person_weight[p] * pax[p, v] for p in person_ids)
                <= max_weight[v]
            )
    ...
    for v in vehicle_ids:
        ...
        model.Add(sum(pax[p, v] for p in person_ids) == 0).OnlyEnforceIf(occupied[v].Not())

        model.Add(sum(op[p, v] for p in person_ids) == 1).OnlyEnforceIf(occupied[v])
        model.Add(sum(op[p, v] for p in person_ids) == 0).OnlyEnforceIf(occupied[v].Not())
...
if __name__ == "__main__":
    demo_balloons = [
        {"id": "B01", "capacity": 3, "allowed_operators": ["pilotA", "pilotB"], "max_weight": 250},
        {"id": "B02", "capacity": 4, "allowed_operators": ["pilotC"], "max_weight": 350},
    ]
    demo_cars = [
