...
def test_person_uniqueness(demo_data):
    ...
    for vid, crew in demo_data["manifest"].items():
        ...
        for op in crew["operator"]:
            assert (
                op not in operator_role
            ), f"{op} operator in {operator_role[op]} and {vid}"
...
def test_frozen_passenger_and_absent():
    ...
    balloons = [{"id": "B", "capacity": 3, "allowed_operators": ["pilot"]}]
    cars = [{"id": "C", "capacity": 2, "allowed_operators": ["driver", "pilot"]}]
    people = [
